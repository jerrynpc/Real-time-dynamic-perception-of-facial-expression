...
def return_name(codelist):
    names = ['你很生气的样子哟','你鄙夷的表情不好看','你憎恶的样子很不好吖','你怎么那么恐惧？','你看起来很开心丫','正常表情','你看起来很伤心呐','你遇到什么惊喜的事情啦？']
...
def return_name_en(codelist):
    names = ['anger','contempt','disgust','fear','happy','normal','sad','surprised']
    for it in range(0, len(codelist), 1):
        if int(codelist[it]) == 1.0:
            return names[it]
...
def face_rec():
    ...
    #camera = cv2.VideoCapture("test.mp4") ## 视频
    ...
    while (True):
        ...
        for i, d in enumerate(dets): # 依次区分截图中的人脸
            ...
            face = cv2.resize(face, (img_h, img_w))
